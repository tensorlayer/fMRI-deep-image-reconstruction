...
def encoder(x, resblk=4, z_dim=100, is_train=True, reuse=False):
    ...
    with tf.variable_scope('encoder', reuse=reuse):
        ...
        # (64, 64, 3)
        net_in = InputLayer(x, name='e_in')

        # (32, 32, 64)
        net = Conv2d(net_in, n_filter=gf_dim, filter_size=filters, strides=stride,
                     act=tf.nn.relu, padding='SAME', W_init=w_init, name='e_n32s2_c0')

        # (16, 16, 128)
        net = Conv2d(net, n_filter=gf_dim * 2, filter_size=filters, strides=stride,
                     act=tf.nn.relu, padding='SAME', W_init=w_init, name='e_n64s2_c0')
        ...
        net = Conv2d(net, n_filter=gf_dim * 4, filter_size=filters, strides=stride,
                     act=tf.nn.relu, padding='SAME', W_init=w_init, name='e_n128s2_c0')
        ...
        net = FlattenLayer(net, name='e_flatten')
        out = DenseLayer(net, n_units=z_dim, act=tf.identity, W_init=w_init, name='e_output')
...
def generator(z, y, h_dim=128, is_train=True, reuse=False):
    ...
    with tf.variable_scope('generator', reuse=reuse):
        ...
        net_in = DenseLayer(net_in, n_units=h_dim, act=tf.identity,
                            W_init=w_init, name='g_h0_lin0')
        # (64*16*4*4 = 4*4*1024)
        net_h0 = DenseLayer(net_in, n_units=gf_dim*16*s16*s16, W_init=w_init,
                            act=tf.identity, name='g_h0_lin1')

        # (4, 4, 1024)
        net_h0 = ReshapeLayer(net_h0, shape=(-1, s16, s16, gf_dim*16), name="g_h0_reshape")

        net_h0 = BatchNormLayer(net_h0, act=tf.nn.relu, is_train=is_train, gamma_init=g_init, name='g_h0_batch_norm')

        # (8, 8, 512)
        net_h1 = DeConv2d(net_h0, n_filter=gf_dim * 8, filter_size=filters, out_size=(s8, s8), strides=stride,
                          padding='SAME', batch_size=batch_size, W_init=w_init, name='g_h1_deconv2d')

        net_h1 = BatchNormLayer(net_h1, act=tf.nn.relu, is_train=is_train, gamma_init=g_init, name='g_h1_batch_norm')

        # (16, 16, 256)
        net_h2 = DeConv2d(net_h1, n_filter=gf_dim * 4, filter_size=filters, out_size=(s4, s4), strides=stride,
                          padding='SAME', batch_size=batch_size, W_init=w_init, name='g_h2_deconv2d')

        net_h2 = BatchNormLayer(net_h2, act=tf.nn.relu, is_train=is_train, gamma_init=g_init, name='g_h2_batch_norm')

        # (32, 32, 128)
        net_h3 = DeConv2d(net_h2, n_filter=gf_dim * 2, filter_size=filters, out_size=(s2, s2), strides=stride,
                          padding='SAME', batch_size=batch_size, W_init=w_init, name='g_h3_deconv2d')

        net_h3 = BatchNormLayer(net_h3, act=tf.nn.relu, is_train=is_train, gamma_init=g_init, name='g_h3_batch_norm')

        # (64, 64, 3)
        out = DeConv2d(net_h3, n_filter=c_dim, filter_size=filters, out_size=(image_size, image_size), strides=stride,
                       padding='SAME', batch_size=batch_size, W_init=w_init, name='g_h4_deconv2d')
...
def discriminator(x_input, y, is_train=True, reuse=False):
    ...
    with tf.variable_scope('discriminator', reuse=reuse):
        ...
        # (64, 64, 3)
        net_in = InputLayer(inputs=x_input, name='d_in')

        # (32, 32, 64)
        net_h0 = Conv2d(net_in, n_filter=df_dim, filter_size=filters, strides=stride,
                        act=tf.nn.leaky_relu, padding='SAME', W_init=w_init, name='d_h0_conv2d')

        # (16, 16, 128)
        net_h1 = Conv2d(net_h0, n_filter=df_dim * 2, filter_size=filters, strides=stride,
                        act=None, padding='SAME', W_init=w_init, name='d_h1_conv2d')

        net_h1 = BatchNormLayer(net_h1, act=tf.nn.leaky_relu, is_train=is_train,
                                gamma_init=gamma_init, name='d_h1_batch_norm')

        # (8, 8, 256)
        net_h2 = Conv2d(net_h1, n_filter=df_dim * 4, filter_size=filters, strides=stride,
                        act=None, padding='SAME', W_init=w_init, name='d_h2_conv2')

        net_h2 = BatchNormLayer(net_h2, act=tf.nn.leaky_relu, is_train=is_train,
                                gamma_init=gamma_init, name='d_h2_batch_norm')

        # (4, 4, 512)
        net_h3 = Conv2d(net_h2, n_filter=df_dim * 8, filter_size=filters, strides=stride,
                        act=None, padding='SAME', W_init=w_init, name='d_h3_conv2')

        net_h3 = BatchNormLayer(net_h3, act=tf.nn.leaky_relu, is_train=is_train,
                                gamma_init=gamma_init, name='d_h3_batch_norm')

        net_h4 = FlattenLayer(net_h3, name='d_h4_flatten')

        # merged_layer = tf.concat(axis=1, values=[net_h4, y])
        # net_h4 = DenseLayer(merged_layer, n_units=512, act=tf.nn.relu, W_init=w_init, name='d/h4/dense_cond')

        out = DenseLayer(net_h4, n_units=1, act=tf.identity, W_init=w_init, name='d_h4_lin_sigmoid')
...
def code_discriminator(z, reuse=False):
    ...
    with tf.variable_scope('code_evaluator', reuse=reuse):
        ...
        net_in = InputLayer(z, name='cd_in')

        net = DenseLayer(net_in, n_units=h, act=tf.nn.leaky_relu, W_init=w_init, name="cd_fc1")

        net = DenseLayer(net, n_units=h, act=tf.nn.leaky_relu, W_init=w_init, name="cd_fc2")

        out = DenseLayer(net, n_units=1, act=tf.identity, W_init=w_init, name="cd_sigmoid")
